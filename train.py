...
# These are all parameters tied to the system model architecture we are using which is Inception v3
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    """
    Builds the image list and trains the image recognition model
    """
    ...
    # The root directory may display first and we skip it.
    is_root_dir = True
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
                           ...
                           bottleneck_tensor):
    """Creating a single bottleneck file"""
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
    ...
    # Organize the following ops as `final_training_ops` so they're easier to display in TensorBoard
...
def main(_):
    # Set the directory to write and save summaries to for TensorBoard
    if tf.gfile.Exists(FLAGS.summaries_dir):
        tf.gfile.DeleteRecursively(FLAGS.summaries_dir)
    tf.gfile.MakeDirs(FLAGS.summaries_dir)

    # Set the pre-trained graph.
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if do_distort_images:
            # We will be applying distortions to the operations we'll need.
